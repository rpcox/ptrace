/* execvp.c
  ...
  Basic usage of ptrace(2) to start a suspended process and hollow it.
...
void parent(pid_t child_pid) {
  ...
  while(waitpid(child_pid, &status, 0) && ! WIFEXITED(status)) {
    ...
    fprintf(stderr, "%6d: child[%d]  %4llu  %s\n", line, child_pid, REG(regs), sys_call(REG(regs)));
    line++;
...
int main(int argc, char* argv[]) {
  ...
  if (argc == 1) {
    fprintf(stderr, "usage: hollow PROGRAM [ OPTIONS ... ]\n");
    exit(1);
  }
